...
const int FFT_steps = 1024;
void fast_fourier_transform (complex double psi [FFT_steps], int number_steps) {
	cout << "fft start\n";
	int length, i, j, n, m;
	double r;
...
void inverse_fast_fourier_transform (complex double psi [FFT_steps], int number_steps) {
	int length, i, j, n, m;
	double r;
	complex double temp, temp1, temp2, wn, w;
	// Reordering of psi
...
void script(){
	...
	double sum = 0, x_0 = QW.getThickness()/2.0;
	double sigma = 10;
	...
	cout << "Conv.\n";
	vector<vector<double>> Potential_QWI(3, vector<double>(2*number_steps));
	vector<double> Gauss_y_new = pad_func_zeros(Gauss_y);
	vector<double> Potential_y_new = pad_func_zeros(Potential_y[0]);
	Potential_QWI[0] = convolution(Gauss_y_new, Potential_y_new);
	cout << "Conv returned.\n";
	Potential_QWI[0] = shiftEdgeToCenter(Potential_QWI[0]);
	// Print convolution result
	double potential1[number_steps*2];//, potential2[number_steps*2];
	double x_test[number_steps*2];
	cout << "Conv returned.\n";
	for(int i = 0; i<number_steps*2; i++){
		x_test[i] = i;
		potential1[i] = Potential_QWI[0][i];
	}
	
    gnuplot_two_functions ("Potentials", "linespoints", "x [nm]", "Effective Potentials",
			x_test, potential1, number_steps*2, "Conduction", x_test, potential1, number_steps*2, "Valence");
