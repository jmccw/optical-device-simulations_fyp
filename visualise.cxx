...
//~ void solve(Heterostructure& heterostructure, std::vector<double>& x_out,
    ...
    //~ for(int p = 0; p<=2; p++){ //for all particle types
		...
		//~ double alpha_w[number_steps], alpha_e[number_steps], alpha_x[number_steps];
		//~ alpha_w[0] = - (1.0/(heterostructure.eff_mass(p,x[0])+heterostructure.eff_mass(p,x[0])));
        //~ alpha_e[0] = - (1.0/(heterostructure.eff_mass(p,x[0])+heterostructure.eff_mass(p,x[1])));
        //~ alpha_x[0] = -alpha_w[0]-alpha_e[0];
        
        //~ M[0][0] = alpha_x[0] + ((delta_x*delta_x)/(2.0*hbar_sqaured_2m))*heterostructure.getPotential(p,x[0],0);
        //~ M[0][1] = alpha_e[0];

        //~ for(int nr = 1; nr < number_steps-1; nr++){
            //~ alpha_w[nr] = - (1.0/(heterostructure.eff_mass(p,x[nr])+heterostructure.eff_mass(p,x[nr-1])));
            //~ alpha_e[nr] = - (1.0/(heterostructure.eff_mass(p,x[nr])+heterostructure.eff_mass(p,x[nr+1])));
            //~ alpha_x[nr] = -alpha_w[nr]-alpha_e[nr];

            //~ M[nr][nr-1] = alpha_w[nr];    //sub-diagonal
            //~ M[nr][nr] = alpha_x[nr] + ((delta_x*delta_x)/(2.0*hbar_sqaured_2m))*heterostructure.getPotential(p,x[nr],nr); //diagonal
            //~ M[nr][nr+1] = alpha_e[nr];   //upper diagonal   
		//~ }

        //~ alpha_w[number_steps-1] = - (1.0/(heterostructure.eff_mass(p,x[number_steps-1])+heterostructure.eff_mass(p,x[number_steps-1-1])));
        //~ alpha_e[number_steps-1] = - (1.0/(heterostructure.eff_mass(p,x[number_steps-1])+heterostructure.eff_mass(p,x[number_steps-1]))); // assuming m(x_edge-dx) = m(x_edge) as boundary condition
        //~ alpha_x[number_steps-1] = -alpha_w[number_steps-1]-alpha_e[number_steps-1];
        //~ M[number_steps-1][number_steps-2] = alpha_w[number_steps-1];
        //~ M[number_steps-1][number_steps-1] = alpha_x[number_steps-1] + ((delta_x*delta_x)/(2.0*hbar_sqaured_2m))*heterostructure.getPotential(p,x[number_steps-1],number_steps-1);
		...
		//cout << "test3 ";
		//for (int i = 0; i < number_steps; ++i) {
		//	energies[p][i] = ((delta_x*delta_x)/(2.0*hbar_sqaured_2m))*eigenvalues(i);			// Assign eigenvalue to energies
		//	for (int j = 0; j < number_steps; ++j) {	// Assign eigenvector to eigenVectors
		//		eigenVectors[p][i][j] = eigenvectors(j, i);
		//	}
		//}
